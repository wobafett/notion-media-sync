...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('notion_igdb_sync.log'),
...
def validate_environment():
    ...
    if not database_id:
        errors.append("NOTION_DATABASE_ID: Your Notion database ID")
    ...
    if len(database_id.replace('-', '')) != 32:
        logger.warning("NOTION_DATABASE_ID format seems incorrect")
...
def main():
    ...
    try:
        ...
        parser = argparse.ArgumentParser(description='Synchronize Notion database with IGDb data')
        ...
        if args.workers < 1:
            ...
            sys.exit(1)
        if args.workers > 4:
            logger.warning(f"Using {args.workers} workers may cause rate limiting issues")
        
        logger.info("Starting Notion IGDb Sync")
